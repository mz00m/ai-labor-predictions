...
const CONTEXT_MAP: Record<string, (v: number) => string> = {
  ...
  "tech-sector-displacement": (v) =>
    `The sector-wide average of ${v > 0 ? "+" : ""}${Number.isInteger(v) ? v : v.toFixed(1)}% obscures a K-shaped labor market split. Software developers aged 22-25 experienced a ~20% employment decline from late 2022 to Sept 2025 (Brynjolfsson, Chandar, Chen 2025), while workers aged 35-49 grew +9% over the same period. The primary mechanism is not mass layoffs but evaporated entry points: firms froze junior hiring as senior developers absorbed routine tasks with AI coding tools. Corroborating signals include a 30% drop in tech internships (Handshake), 25% fewer entry-level tech postings (SignalFire), and CS graduate unemployment at 7.2% vs. 4.5% for all graduates (ACS 2024). Meanwhile, BLS projects overall software developer employment to grow 17.9% through 2033 — the gains are just concentrating among experienced workers who can effectively leverage AI.`,
