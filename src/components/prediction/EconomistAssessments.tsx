...
export default function EconomistAssessments() {
  return (
    <section className="mb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {ECONOMIST_VIEWS.map((e) => {
          ...
          return (
            <div
              key={e.name}
              className="rounded-xl border border-card p-5 flex flex-col"
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-md font-bold text-[#2E3650]">
                    {e.name}
                  </h3>
                ...
                <span
                  className={`text-2xs font-bold uppercase tracking-wider px-2 py-0.5 rounded ${style.bg} ${style.text}`}
                >
                  {e.stanceLabel}
                </span>
              </div>

              <p className="text-sm text-[var(--muted)] leading-relaxed flex-1 mb-3">
                {e.summary}
              </p>
      ...
      <div className="mt-6 rounded-xl border border-card p-5 bg-black/[0.01]">
        <h3 className="text-md font-bold text-[#2E3650] mb-2">
          Where All Six Converge
        </h3>
        <ul className="space-y-1.5 text-sm text-[var(--muted)] leading-relaxed">
          <li>
